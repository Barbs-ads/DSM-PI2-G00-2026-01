...
async function carregarMural(f = {}) {
  ...
  m.innerHTML = '<div class="mural-load"><div class="spinner" style="color:var(--terracota);width:36px;height:36px;border-width:3px"></div><p>Carregando…</p></div>';
...
function renderMural(lista) {
  ...
  if (c) c.textContent = `${lista.length} cartinha${lista.length !== 1 ? "s" : ""}`;
  if (!lista.length) {
    m.innerHTML = '<div class="mural-vazio"><i class="bi bi-envelope-open"></i><p>Nenhuma cartinha encontrada.</p></div>';
    return;
  }
  const badges = {
  ...
  m.innerHTML = lista.map((c, i) => {
    const nomeCrianca = c.nome_crianca || c.crianca_nome || '—';
    const idade       = calcIdade(c.nascimento || c.crianca_data_nasc);
    const label       = PRESENTE[c.presente] || PRESENTE[c.categoria_slug] || c.categoria_nome || c.presente || '—';
    ...
    return `
      <article class="mural-card entra" style="transition-delay:${(i % 4) * 0.07}s">
        <div class="mc-topo ${tom}">
          ...
          <div class="mc-nome">${nomeCrianca}</div>
...
function initFiltros() {
  ...
  const ap = async () => {
    ...
    let l = [..._lista];
    if (selP?.value) {
      l = l.filter((c) => (c.presente || c.categoria_slug) === selP.value);
    }
    if (inp?.value.trim()) {
      const q = inp.value.toLowerCase();
      l = l.filter((c) =>
        (c.nome_crianca || c.crianca_nome || '').toLowerCase().includes(q) ||
        c.texto.toLowerCase().includes(q) ||
        (PRESENTE[c.presente || c.categoria_slug] || '').toLowerCase().includes(q)
      );
    }
    renderMural(l);
  };
  ...
  selS?.addEventListener("change", async () => { _lista = []; await ap(); });
}
...
async function abrirModal(id) {
  ...
  // Busca pontos reais do banco
  let pontosOpts = '';
  try {
    const pontosData = await api('/pontos');
    const pontos = Array.isArray(pontosData) ? pontosData : pontosData.pontos || [];
    pontosOpts = pontos
      .filter(p => p.ativo !== false)
      .map(p => `<option value="${p.id}">📍 ${p.nome} — ${p.endereco}</option>`)
      .join('');
  } catch (e) {
    pontosOpts = '<option value="">Erro ao carregar pontos</option>';
  }
  ...
  document.getElementById("modal-body").innerHTML = `
    <div style="text-align:center;margin-bottom:var(--sp-4)">
      <div style="width:72px;height:72px;background:linear-gradient(135deg,var(--terracota-esc),var(--terracota));border-radius:var(--r-4);display:flex;align-items:center;justify-content:center;margin:0 auto var(--sp-3)">
        <i class="bi bi-envelope-heart-fill" style="font-size:2rem;color:var(--branco)"></i>
      </div>
      <h2 style="font-family:var(--f-display);color:var(--mogno);font-size:var(--t-lg);margin-bottom:4px">${nomeCrianca}</h2>
      <p style="color:var(--mogno-claro);font-size:.84rem">${idade} anos · ${c.inst_nome}</p>
    </div>
...
function initUpload() {
  ...
  inp.addEventListener("change", () => { if (inp.files[0]) prevFoto(inp.files[0]); });
}
